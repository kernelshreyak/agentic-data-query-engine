...
def list_tasks(data:DirectQueryRequest, db: Session = Depends(get_db)):
    """
    Perform a direct query on the specified data source using the provided user query.

    This endpoint retrieves the data source summary for the given data source ID and utilizes
    a language model to generate a response based on the user query.

    Args:
        data (DirectQueryRequest): The direct query request containing the data source ID and user query.
        db (Session): SQLAlchemy session for database operations.

    Returns:
        dict: The response generated by the language model in JSON format.
    """

    datasource = db.query(DataSourceModel).filter(DataSourceModel.datasource_id == data.data_source_id).first()
    llm = LLM(model="gpt-4o",api_key=os.environ["OPENAI_API_KEY"],response_format=DirectQueryResponse)
...
@app.get("/query/agentic")
def list_tasks(analysis_request: AnalysisRequest, db: Session = Depends(get_db)):
    # initiate background task for the agentic query phase

    task = TaskModel(task_type="query", data_source_id=analysis_request.data_source_id)
    q = Queue(connection=Redis())
    job = q.enqueue(analysis_task_execution,analysis_request.user_query, analysis_request.data_source_id)
    task.rq_job_id = job.id
    db.add(task)
    db.commit()
    return {
        "message": "Data source querying task started", 
        "data_source_id": analysis_request.data_source_id,
        "task_id": task.task_id
    }
